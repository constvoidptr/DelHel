...
validation CDelHel::ProcessFlightPlan(EuroScopePlugIn::CFlightPlan& fp, bool nap, bool validateOnly)
{
	...
	if (validateOnly) {
		...
		if (assignedRwy.empty() || hasCustomRwyConfig) {
		...
		if (nap && sidinfo.nap != "") {
			this->LogDebugMessage("--> Assigned sid/rwy: " + sidinfo.nap + "/" + assignedRwy, cs);
			sssid << sidinfo.nap;
		}
		else {
			this->LogDebugMessage("--> Assigned sid/rwy: " + sidinfo.dep + "/" + assignedRwy, cs);
			sssid << sidinfo.dep;
		}
		sssid << "/" << assignedRwy;
